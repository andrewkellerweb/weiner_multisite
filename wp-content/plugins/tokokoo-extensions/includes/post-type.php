...
function tokokoo_register_post_types() {
	...
	/* Load the video post type file if supported. */
	if ( in_array( 'video', $type[0] ) ){
		require_once( trailingslashit( TOKOKOO_EXTENSIONS_INCLUDES ) . 'video-type/video.php' );
	}

	/* Load the event post type file if supported. */
	if ( in_array( 'event', $type[0] ) ){
		require_once( trailingslashit( TOKOKOO_EXTENSIONS_INCLUDES ) . 'event-type/event.php' );
	}

	/* Load the features post type file if supported. */
	if ( in_array( 'features', $type[0] ) ){
		require_once( trailingslashit( TOKOKOO_EXTENSIONS_INCLUDES ) . 'features-type/features.php' );
	}

	/* Load the testimonials post type file if supported. */
	if ( in_array( 'testimonials', $type[0] ) ){
		require_once( trailingslashit( TOKOKOO_EXTENSIONS_INCLUDES ) . 'testimonials-type/testimonials.php' );
	}

	/* Load the photo post type file if supported. */
	if ( in_array( 'photo', $type[0] ) ){
		require_once( trailingslashit( TOKOKOO_EXTENSIONS_INCLUDES ) . 'photo-type/photo.php' );
	}
